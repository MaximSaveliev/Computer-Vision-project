...
class Camera_color_detect_click(QWidget):
    ...
    def run(self):
        ...
        while self.status:
            ...
            if clicked_color is not None:
                color_name = Thread.convert_rgb_to_names((int(clicked_color[2]), int(clicked_color[1]), int(clicked_color[0])))
                ...
                cv.rectangle(frameCopy, (text_x - 10, 115), (text_x + text_width + 7, 50 + text_height - 5), (255, 255, 255), -1)
                cv.putText(frameCopy, color_name, (text_x, 100), 0, 1, (0, 0, 0), 6)
                cv.putText(frameCopy, color_name, (text_x, 100), 0, 1, (int(clicked_color[0]), int(clicked_color[1]), int(clicked_color[2])), 4)
            cv.imshow('Project', frameCopy)
            cv.namedWindow("Project")
            cv.setMouseCallback('Project', Thread.color_detecting)
            key = cv.waitKey(1)
            if key == 27:
                break
