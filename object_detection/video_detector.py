...
class Video_Detector_obj(QWidget): 
    def __init__(self, videoPath, configPath, modelPath, classesPath, grid_video_detect, video_play_btn, video_stop_btn, video_screenshot_btn):
        super().__init__()
        self.videoPath = videoPath
        self.configPath = configPath
        self.modelPath = modelPath
        self.classesPath = classesPath
        self.video_play_btn = video_play_btn
        self.video_stop_btn = video_stop_btn
        ...
        video_play_btn.clicked.connect(partial(self.start, video_play_btn, video_stop_btn))
        video_stop_btn.clicked.connect(self.kill_thread)
        video_screenshot_btn.clicked.connect(self.make_screenshot)
        video_stop_btn.setEnabled(False)
        #self.th.set_files(videoPath, configPath, modelPath, classesPath)
    ...
    @Slot()
    def set_models(self, videoPath, configPath, modelPath, classesPath):
        self.th.set_files(videoPath, configPath, modelPath, classesPath)
    ...
    @Slot()
    def start(self, video_play_btn, video_stop_btn):
        print("Starting...")
        video_play_btn.setEnabled(False)
        video_stop_btn.setEnabled(True)
        self.th.set_files(self.videoPath, self.configPath, self.modelPath, self.classesPath)
        self.th.start()
